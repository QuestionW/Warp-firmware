...
writeSensorRegisterINA219(uint8_t deviceRegister, uint8_t payload_MSB, uint8_t payload_LSB, uint16_t menuI2cPullupValue)
{
	...
	commandByte[0] = deviceRegister;
        payloadByte[0] = payload_MSB;
        payloadByte[1] = payload_LSB;
...
printSensorDataINA219()
{
	...
	// Set pointer address to Bus Voltage (0x02 from data sheet)
        writeSensorRegisterINA219(0x02,0x00,0x00,1);
	...
	readSensorRegisterValueCombined = ((readSensorRegisterValueMSB & 0xFF) << 5) | ((readSensorRegisterValueLSB & 0xF8) >> 2);
        //SEGGER_RTT_printf(0, "\nBus Voltage Reading MSB: %d", readSensorRegisterValueMSB);
        //SEGGER_RTT_printf(0, "\nBus Voltage Reading LSB: %d", readSensorRegisterValueLSB);
        //SEGGER_RTT_printf(0, "\nreadSensorRegisterValueMSB & 0xFF: %d", readSensorRegisterValueMSB & 0xFF);
        //SEGGER_RTT_printf(0, "\nreadSensorRegisterValueLSB & 0xF8: %d", readSensorRegisterValueLSB & 0xF8);
        //(0, "\n(readSensorRegisterValueMSB & 0xFF) << 5: %d", (readSensorRegisterValueMSB & 0xFF) << 5);
        //SEGGER_RTT_printf(0, "\nreadSensorRegisterValueLSB & 0xF8) >> 2: %d", (readSensorRegisterValueLSB & 0xF8) >> 2);
        //SEGGER_RTT_printf(0, "\nreadSensorRegisterValueCombined %d", readSensorRegisterValueCombined);
    ...
    for (i = 0; i < 1000; ++1)
    {
        ...
        SEGGER_RTT_printf(0, "\n%d", int(readSensorRegisterValueCombined * 0.1));
    }
